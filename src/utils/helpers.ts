...
export function titleize(hyphenedString: string): string {
  if (process.env.NODE_ENV !== 'production') {
    if (typeof hyphenedString !== 'string' || hyphenedString.length <= 0) {
      console.error(
        'titleize(hyphenedString) expects a non empty string argument.'
      );
    }
  }
...
function includePeerDependencies(
  ...
  if ((window as any).muiDocConfig) {
    newDeps = (window as any).muiDocConfig.csbIncludePeerDependencies(newDeps, {
      versions,
    });
  }
...
export function getDependencies(
  ...
  const { codeLanguage = 'JS', muiCommitRef } = options;
  ...
  let versions: Record<string, string> = {
    ...
    '@material-ui/styled-engine': getMuiPackageVersion(
      'styled-engine',
      muiCommitRef
    ),
    '@material-ui/styled-engine-sc': getMuiPackageVersion(
      'styled-engine-sc',
      muiCommitRef
    ),
    '@material-ui/styles': getMuiPackageVersion('styles', muiCommitRef),
    '@material-ui/system': getMuiPackageVersion('system', muiCommitRef),
    '@material-ui/private-theming': getMuiPackageVersion(
      'theming',
      muiCommitRef
    ),
