      ...
      editor.windowManager.open({
        ...
        body: {
          ...
          items: [
            {
              type: 'htmlpanel',
              html: '<div style="text-align: right; direction: rtl">فرمول LaTeX موردنظر را در این کادر وارد کنید. (<a target="_blank" href="http://www.hostmath.com/">راهنما</a>)</div>',
            },
            {
              type: 'textarea',
              name: 'latex',
        ...
        onChange: (api: any) => updateSvg(api.getData().latex),
      });
      const updateSvg = (latex: string) => {
        latexData = latex;
        svgLatex = mathTextToSvg(latexData);
        const pluginPreview = document.querySelector('.latex-plugin-preview');

        if (pluginPreview) {
          pluginPreview.innerHTML = svgLatex.outerHTML;
        }
      };
    };

    const register = (editor: any /* TODO:fix */) => {
      editor.addCommand('latexEditor', function () {
        open(editor);
      });
    };

    const register$1 = (editor: any /* TODO:fix */) => {
